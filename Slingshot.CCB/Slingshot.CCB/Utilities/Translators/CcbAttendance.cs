...
namespace Slingshot.CCB.Utilities.Translators
{
    public static class CcbAttendance
    {
        public static List<Attendance> Translate( XElement inputAttendance, List<EventDetail> eventDetails )
        {
            ...
            var occurrenceElementValue = inputAttendance.Element( "occurrence" ).Value;
            if ( !string.IsNullOrWhiteSpace( occurrenceElementValue ) )
            {
                var attendanceDate = occurrenceElementValue.AsDateTime().Value;
